#This is the file to change jump point
...
#-----------------------------------OG scene-----------------------------------#
label NOP1:                     #Beginning, heart attack, Arrhythmia, blablabla.
    call iscene ("NOP1")
    jump_out op_video
...
label NOP2:                     #Hospital, 4 month after the heart attack
    call iscene ("NOP2")
    jump_out tc_act1
...
label A1:                       #1rst day at school
    call iscene ("A1")
    call imenu ("choiceA1")     #"Do you want to introduce yourself to the class" Choice

    if _return == m1:           #"Why ?"

        call iscene ("A1a")     #you don't introduce
    else:                       #"Yeah, of course"

        $ attraction_sc += 1    #Shizune ? attraction toward you ++
        call iscene ("A1b")     #you introduce
    call iscene ("A1c")         #Rest of the day, 
    jump_out A2

label A2:
    call iscene ("A2")
    if seen_scene ("A1a"):      #Clever trick from the dev I think, if we saw the scene "A1a" (don't introduce)
        call iscene ("A2a")     #Do the scene where you don't introduce
    else:                       #else (we don't need to bother to precise "if we saw the scene A1b" because if the a isn't seen, either you seen the a, or you are an dirty hacker lol)
        call iscene ("A2b")     #Do the scene where you introduce
    call iscene ("A2c")         #Then continue the day
    if seen_scene ("A1b"):      #if you introduce
        call iscene ("A2d")     #Misha compliment you
    else:                       #else 
        call iscene ("A2e")     #she look at you weird
    call iscene ("A2f")         #And we talk/do the assignment 
    jump_out A3
